...
# THE  BELOW IP RANGE SHOULD BE THE IP RANGE OF THE EXTERNAL NETWORK THAT IS USED TO CONNECT TO THE INTERNET
...
NEUTRON_PASS=admin
HEAT_PASS=admin
RABBIT_GUEST_PASS=admin
RABBIT_OS_PASS=admin
CINDER_PASS=admin
MAGNUM_PASS=admin
...
# leave 4GB of RAM for controller functions
...
mysqladmin -p$MYSQL_PASS create cinder
mysql -u root -p$MYSQL_PASS -e "GRANT ALL PRIVILEGES ON cinder.* TO 'cinder'@'localhost' IDENTIFIED BY '$CINDER_PASS';"
mysql -u root -p$MYSQL_PASS -e "GRANT ALL PRIVILEGES ON cinder.* TO 'cinder'@'%' IDENTIFIED BY '$CINDER_PASS';"

openstack user create --domain default --password $CINDER_PASS cinder || fail
openstack role add --project service --user cinder admin || fail

openstack service create --name cinder \
  --description "OpenStack Block Storage" volume || fail

openstack service create --name cinderv2 \
  --description "OpenStack Block Storage"  volumev2 || fail

openstack endpoint create --region RegionOne \
  volume public http://$IP:8776/v1/%\(tenant_id\)s || fail

openstack endpoint create --region RegionOne \
  volume internal http://$IP:8776/v1/%\(tenant_id\)s || fail

openstack endpoint create --region RegionOne \
  volume admin http://$IP:8776/v1/%\(tenant_id\)s || fail

openstack endpoint create --region RegionOne \
  volumev2 public http://$IP:8776/v2/%\(tenant_id\)s || fail

openstack endpoint create --region RegionOne \
  volumev2 internal http://$IP:8776/v2/%\(tenant_id\)s || fail

openstack endpoint create --region RegionOne \
  volumev2 admin http://$IP:8776/v2/%\(tenant_id\)s || fail


apt-get install cinder-api cinder-scheduler python-cinderclient \
  lvm2 cinder-volume python-mysqldb -y
...
vgcreate "cinder-volumes" /dev/loop2 || fail
...
rabbit_password = $RABBIT_OS_PASS
[database]
connection = mysql+pymysql://cinder:$cinder_PASS@$IP/cinder
[keystone_authtoken]
auth_uri = http://$IP:5000
auth_url = http://$IP:35357
auth_plugin = password
